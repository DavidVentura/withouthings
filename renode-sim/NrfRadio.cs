...
{
    public class NrfRadio : BasicDoubleWordPeripheral, IRadio, IKnownSize, INRFEventProvider
    {
        public NrfRadio(IMachine machine) : base(machine)
        {
            IRQ = new GPIO();
            // The nRF timers count on machine.ClockSource, so an event scheduled there
            // fires while that clock is being advanced and a PPI-driven TIMER capture in
            // the callback reads the timer at the instant of the event. Events posted
            // through LocalTimeSource.ExecuteInSyncedState run outside that advance, so
            // while the CPU sleeps the timers still hold their pre-event value and an END
            // capture reads hundreds of microseconds early.
