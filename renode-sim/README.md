...
```bash
python3 abi/classify_words.py                          # the word classes, which the peripheral names and the index read
...
python3 abi/accesses.py; python3 abi/periphnames.py --apply
python3 abi/symbols.py --check; python3 abi/coverage.py
```

`python3 abi/peripherals.py [--at 0x40007004] [--words]` reads NRF52840.svd
and says what a peripheral address is called; `python3 abi/accesses.py
[--report NAME] [--vertical SAADC] [--vertical-all] [--run]` writes
`abi/out/accesses.json`, the per-function and per-object access index, and
`--run` cross-checks it against five scenarios with Renode's peripheral access
log on.
