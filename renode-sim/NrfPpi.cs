//
// Renode's stock NRF52840_PPI defers every task trigger into the time source's
// synced-state queue. A trigger raised from a timer callback (radio END/DISABLED,
// TIMER0 compare) then runs after the CPU has moved on, so a PPI-driven TIMER0
// capture reads a stale count and the SoftDevice's radio schedule (anchored on
// that capture) is off by a whole packet time. Tasks are written immediately.
//
//
...
{
    public sealed class NrfPpi : BasicDoubleWordPeripheral, IKnownSize
    {
        public NrfPpi(IMachine machine) : base(machine)
        {
