...
BAND = 12
# A body this small is a shape rather than a function; abi/libc_check.py refuses
...
def main():
    ...
    for v in accepted.values():
        st = streams[v["stream"]]
        links, broken = [], 0
        # How many calls the image body makes, against how many the reference
        # makes. The archive brings a body's callees with it, so a body that
        # calls less than the reference does is not that body; calling more is
        # the image's own source carrying a path the reference does not, which
        # the byte verdict reports on its own.
        start = st["addrs"][v["pos"]]
        v["ref_calls"] = len(v["fn"]["calls"])
        v["image_calls"] = sum(
            1 for a, tok in zip(st["addrs"], st["tokens"])
            if start <= a < start + v["fn"]["size"]
            and tok.split(" ")[0].split(".")[0] in ("bl", "blx"))
        for i, callee in v["fn"]["calls"].items():
            j = v["pos"] + i
            tgt = st["targets"][j] if j < len(st["targets"]) else None
    ...
    # A body the bytes do not settle is carried by the token alignment alone,
    # which is a shape, and a body this short has no shape of its own: under
    # SHAPE_BYTES the reference has a prologue, the calls, a flag store and a
    # return, and
    # `void f(void) { g(); flag = 0; }` is what every library's uninit wrapper
    # looks like. The only structure such a body has is how many calls it makes,
    # so that has to agree. Two SDK uninit wrappers scored 0.875 and 0.70 over 8
    # and 10 instructions against the same 14-byte image body, which makes one
    # call -- the same one for both -- where they make two and four.
    # A longer body carries its own evidence and the byte verdict reports it,
    # so a call the image's source does not have does not refuse the name:
    # prvUnlockQueue is 122 bytes and calls eight of the reference's nine.
    refused = {}
    for name, m in sorted(resolved.items()):
        if (m["verdict"] != "token_only" or m["fn"]["size"] >= SHAPE_BYTES
                or m["image_calls"] == m["ref_calls"]):
            continue
        refused[name] = m
        counts[m["verdict"]] -= 1
    for name, m in sorted(refused.items()):
        print("  REFUSE %-28s 0x%-7x %d bytes with no shape of its own: it makes"
              " %d calls where the reference makes %d"
              % (name, m["address"], m["fn"]["size"], m["image_calls"],
                 m["ref_calls"]))
        del resolved[name]
    ...
    # matcher's to decide.
    try:
        # A body the reference reproduces byte for byte is settled; a
        # token alignment is not, and a hand reading may still stand at
        # its address.
        settled = set(m["address"] for m in resolved.values()
                      if m["verdict"] in ("exact", "masked"))
...
def write_matches(path, accepted, best, streams, protos, threshold, acc,
                  variants, ties, verdicts, refused):
    lines = [
    ...
    lines += ["", "shape_refused:  # a token alignment onto a body with no shape"
              " of its own, which the call count refutes"]
    for name, m in sorted(refused.items()):
        lines.append("  - {name: %s, address: 0x%x, bytes: %d, calls: %d,"
                     " reference_calls: %d}"
                     % (name, m["address"], m["fn"]["size"], m["image_calls"],
                        m["ref_calls"]))
