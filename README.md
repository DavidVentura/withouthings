...
- Automatic activities (walking, running), segmentation is implemented locally instead of using the API so it's a bit
  different
...
Under `renode-sim/` there's a Renode based simulator that can run the stock firmware, and attempts to name the symbols,
dependencies, etc.

# DEBUG DATA


There's a stream of `DEBUG` data which holds some "unknown" data at a high sampling rate (mostly 1 Hz), which are not
used for anything as far as I'm aware
...
On the original app, this data is sent straight to withings. I'm not sure why they need 1Hz raw sensor data but I don't
like it.
